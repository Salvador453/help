...
# ТВОЙ API КЛЮЧ (логин:пароль) → кодируем в Base64 для Basic Auth
API_KEY = "14d49bd6:19c6d5a643e2fddfb2a473e9c4c08ccd"
encoded_key = base64.b64encode(API_KEY.encode()).decode()

# API с детализацией по ТГ
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(API_URL, headers=headers, timeout=10) as response:
                    if response.status == 401:
                        logger.error("❌ Помилка авторизації! Невірний API ключ")
                    ...
                    # API возвращает список активных тревог
                    for alert in data:
                        region_id = str(alert.get("regionId", ""))
                        location = alert.get("locationTitle", "")
                        
                        if region_id == ZAPORIZHZHIA_CITY_ID or "Запоріжжя" in location:
                            is_active_now = True
